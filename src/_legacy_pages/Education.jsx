const programs = [
  {
    ...
    icon: '🎓',
    duration: '3~6개월',
    departments: ['ICT 정책 전문가 과정', '디지털 경제 분석 과정', '방송통신 규제 과정', '데이터 거버넌스 과정'],
  ...
  },
  {
    ...
    icon: '📜',
    duration: '1~3개월',
    departments: ['디지털 정책 입문', 'AI·빅데이터 정책 이해', '사이버보안 정책 과정'],
  ...
  },
  {
    ...
    icon: '🌐',
    duration: '2~4주',
    departments: ['해외 ICT 정책 벤치마킹', '국제 디지털 협력 과정', '개도국 ICT 정책 지원'],
    desc: '국제 ICT 정책 협력 및 개발도상국 역량 강화를 위한 국제 연수 과정',
  },
]

const onlinePrograms = [
  { title: 'ICT 정책 입문', duration: '6주', enrolled: '8,500+', level: '입문' },
  { title: '디지털 경제의 이해', duration: '8주', enrolled: '6,200+', level: '중급' },
  { title: '데이터 정책과 개인정보보호', duration: '6주', enrolled: '9,100+', level: '입문' },
  { title: '6G 시대의 통신 정책', duration: '4주', enrolled: '3,800+', level: '고급' },
]
    ...
    <div className="bg-white dark:bg-gray-900 min-h-screen">
      <div className="bg-gradient-to-r from-kisdi-dark to-kisdi-red py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-white/85 text-lg">ICT 정책 전문가를 육성하는 교육을 제공합니다</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-20">
        {/* Training Programs */}
        <section>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-8">교육 과정</h2>
          <div className="grid md:grid-cols-3 gap-6">
            {programs.map((p) => (
              <div key={p.level} className="border border-gray-200 dark:border-gray-700 rounded-2xl p-6 bg-white dark:bg-gray-800 border-t-4 border-t-kisdi-red">
                <div className="flex items-center gap-3 mb-4">
                  <span className="text-3xl">{p.icon}</span>
                  <div>
                    <h3 className="font-bold text-gray-900 dark:text-white text-xl">{p.level}</h3>
                    <span className="text-sm text-gray-500 dark:text-gray-400">기간: {p.duration}</span>
                  </div>
                </div>
            ...
            {onlinePrograms.map((course) => (
              ...
              >
                <div className="w-14 h-14 bg-kisdi-red/10 dark:bg-red-900/30 rounded-xl flex items-center justify-center flex-shrink-0">
                  <span className="text-kisdi-red dark:text-red-400 font-bold text-lg">K</span>
                </div>
